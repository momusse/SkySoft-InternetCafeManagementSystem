...
using InternetCafeManagementSystem.Api;

// ══════════════════════════════════════════════════════════
// SKYSOFT INTERNET CAFE MANAGEMENT SYSTEM
// Runs both the console app AND the Web API simultaneously.
// The Web API allows the customer-facing frontend (customer-portal.html)
// to read and write data to the same SQL database as the console app.
// ══════════════════════════════════════════════════════════

// Initialise the shared service layer — loads all data from SQL database on startup
InternetCafeService cafe = new InternetCafeService();

// ── Start Web API in background thread ──────────────────────
// This lets the console app menu keep running while the API
// listens for HTTP requests from the frontend on port 5000.
Thread apiThread = new Thread(() =>
{
    CafeApiServer.Start(cafe);
});
apiThread.IsBackground = true;
apiThread.Start();

Console.WriteLine("✓ Web API started on http://localhost:5000");
Console.WriteLine("  Customer portal (customer-portal.html) can now connect to live data.");
Console.WriteLine();
